...
操作インターフェースと常駐プロセスの通信方式は、OS別アダプターに分離します。
macOSとLinuxではUnix domain socket、WindowsではNamed Pipeを候補とします。
...
常駐プロセスとIPCは、現在のMVPには含めません。
コア層の状態機械を検証した後に、アプリケーション層の次期機能として追加します。
